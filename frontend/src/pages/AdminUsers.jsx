...
export default function AdminUsers() {
	...
	return (
		<Container>
			<div className={`space-y-6 min-h-screen ${isDark ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900' : 'bg-gray-50'} transition-colors p-4`}>
				<header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
					<div>
						<h1 className={`text-2xl font-bold transition-colors ${isDark ? 'text-slate-50' : 'text-gray-900'}`}>Admin • Users</h1>
					...
					<div className="flex items-center gap-3">
						<input
							...
							className={`w-full sm:w-64 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 transition-colors ${isDark ? 'bg-slate-700 border-slate-600 text-slate-50 placeholder-slate-400' : 'border-gray-300 bg-white text-gray-900'}`}
						/>
						<button
										...
										{filtered.map((u) => (
											<div key={u._id} className={`p-4 border rounded-lg transition-colors ${isDark ? 'bg-slate-700 border-slate-600' : 'border-gray-200'}`}>
												<div className="flex items-center justify-between">
													<div>
														<div className={`font-semibold transition-colors ${isDark ? 'text-slate-50' : ''}`}>{u.name || '—'}</div>
														<div className={`text-sm transition-colors ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>{u.email || '—'}</div>
													</div>
													<div className="text-right">
														...
														<div className={`text-xs transition-colors ${isDark ? 'text-slate-500' : 'text-gray-500'}`}>{new Date(u.createdAt).toLocaleDateString()}</div>
													</div>
												</div>
												<div className="flex items-center justify-between mt-3 mb-3">
													<div className={`text-sm transition-colors ${isDark ? 'text-slate-300' : ''}`}>Tasks: {u.tasksCompleted || 0}</div>
													<span className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors ${u.role === 'admin' ? (isDark ? 'bg-purple-900 text-purple-300' : 'bg-purple-100 text-purple-800') : (isDark ? 'bg-blue-900 text-blue-300' : 'bg-blue-100 text-blue-800')}`}>
														{u.role || 'user'}
													</span>
												</div>
			...
			{editingRole && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
					<div className={`rounded-lg p-6 max-w-md w-full mx-4 transition-colors ${isDark ? 'bg-slate-800 border border-slate-700' : 'bg-white'}`}>
...
function StatCard({ title, value, isDark }) {
	return (
		<div className={`p-5 rounded-lg border transition-colors ${isDark ? 'bg-gradient-to-r from-slate-700 to-slate-800 border-slate-600' : 'bg-gradient-to-r from-indigo-50 to-white border-gray-200'}`}>
			<div className={`text-sm transition-colors ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>{title}</div>
			<div className={`mt-2 text-2xl font-bold transition-colors ${isDark ? 'text-slate-50' : 'text-gray-900'}`}>{value}</div>
		</div>
	)
}
